...
figure,ax=plt.subplots( math.ceil(len(files)/powerbase), powerbase)


for chartNum, filename in enumerate(files[::-1]):
    ...
    ax[chartNum // powerbase, chartNum % powerbase].set_xlabel(param+" for sequence lenght: "+str((int(filename.split("/")[-1].rstrip(".csv").lstrip(param))+1)*200), fontsize=20)

for chartNum in range(len(files), pow(powerbase,2)):
    if chartNum // powerbase >= math.ceil(len(files)/powerbase):
        break
